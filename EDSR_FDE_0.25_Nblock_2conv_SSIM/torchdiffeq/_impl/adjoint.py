...
"""
反向过程也可以被 Pytorch 在内部自动实现了 （autograd backpropagate），
而作者提出用 adjoint sensitivity method。
作者指出使用 adjoint 方法可将内存复杂度降为O(1)
"""
"""
 首先说明了odeint_adjoint 的变量是有序的，然后通过内部类封装了一下 func，这里明确的限制了 func 是 nn.Module，
 这样 ODE-Net 的前向过程就实现了。接下来，通过 OdeintAdjointMethod 具体执行 ODE 的前向和反向过程：
"""
    ...
    def backward(ctx, *grad_output):
        ...
        def augmented_dynamics(t, y_aug):
            ...
            with torch.set_grad_enabled(True):
                ...
                func_eval = func(t, y)
                #torch.autograd.grad是用来计算输出对输入的梯度
...
"""
odeint_adjoint 简单第封装了 odeint，并实现了反向过程。但其最大的缺憾（硬伤）是func f f 
的取值必须是 nn.Module 的方法  ,这是为了收集微分方程的参数。
 Why must be collect parameters of the differential equation? The answer is use to backward of adjoint odeint.
"""
