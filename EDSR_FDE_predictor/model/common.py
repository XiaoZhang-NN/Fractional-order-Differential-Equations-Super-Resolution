    ...
    def forward(self, x):
        ...
        for i in range (self.num_for):
            ...
            for m in range(i+1):
                ...
                ff[m] = self.conv1(self.relu1(yn[m]))

                # self.f = self.f + a[m] * ff[m]
                self.yn_p = self.yn_p + ff[m] * b[m]
            
            # f_p = self.conv1(self.relu1(yn[0] + self.gamma_1[self.alpha_index]*self.yn_p))
            yn[i+1] = yn[0] + self.gamma_1[self.alpha_index]*self.yn_p  #yn[0] + self.gamma_1[self.alpha_index]* (self.f + self.a1 * f_p) 
