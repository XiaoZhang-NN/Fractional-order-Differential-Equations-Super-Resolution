...
"""
 ode45表示采用四阶-五阶Runge-Kutta算法，它用4阶方法提供候选解，
 5阶方法控制误差，是一种自适应步长（变步长）的常微分方程数值解法，其整体截断误差为(Δx)^5。解决的是Nonstiff(非刚性)常微分方程。
 与显式龙格库塔方法不同，隐式龙格库塔方法在每一步的计算里需要求解一个线性方程组，这相应的增加了计算的成本
 显式龙格库塔法一般来讲不适用于求解刚性方程。这是因为显式龙格库塔方法的稳定区域被局限在一个特定的区域里
ODE/simulink汇集了各种求解常微分方程数值解的方法，可分为两类：可变补偿类算法和固定步长类算法
可变步长类算法（variable-step）：在解算模型时可以自动调整步长，并通过减小步长来提高计算的精度
"""
"""
以懒著称的数学家发明了根据误差自动调整间隔的自适应方法
